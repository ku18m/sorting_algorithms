...
/**
 *helper - quicksort algorithm helper.
 *@array: array
 *@first: first array element
 *@last: last array element
 *@size: array size
 */
void helper(int *array, ssize_t first, ssize_t last, int size)
{
	ssize_t position = 0;
	...
	if (first < last)
	{
		...
		helper(array, first, position - 1, size);
		helper(array, position, last, size);
	}
}
